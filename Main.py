...
def post_to_endpoint(excel_bytes: bytes):
    files = {
        "edt": (
            "rechazos.xlsx",
            excel_bytes,
...
def flujo_pre_bcp_txt():
    ...
    st.write(
        "Extrae Registro N del PDF, multiplica por 2 internamente, lee línea del TXT y genera Excel."
    )
    ...
    if pdf_file and txt_file:
        with st.spinner("Procesando..."):
            ...
            for page in fitz.open(stream=pdf_file.read(), filetype="pdf"):
                text += page.get_text()
            regs = sorted(
                {int(m) for m in re.findall(r"Registro\s+(\d{1,5})", text, re.IGNORECASE)}
            )
    ...
    if zip_file:
        with st.spinner("Procesando ZIP..."):
            ...
            for _, r in df_proc.iterrows():
                ...
                code, desc = (
                    RECH_ZIP[0]
                    if any(k in o.lower() for k in KEYS_NO_TIT)
                    else RECH_ZIP[1]
                )
                rows.append(
                    {
    ...
    if pdf_file and ex_file:
        with st.spinner("Procesando PRE..."):
            ...
            for page in fitz.open(stream=pdf_file.read(), filetype="pdf"):
                text += page.get_text()
            filas = sorted(
                {int(n) + 1 for n in re.findall(r"Registro\s+(\d+)", text, re.IGNORECASE)}
            )
    ...
    if pdf_file and ex_file:
        with st.spinner("Procesando POST..."):
            ...
            for row in ws.iter_rows(min_row=2, values_only=False):
                if any(str(cell.value) in docs for cell in row):
            ...
            excel_all = df_to_excel_bytes(df)
            if st.button("Validar", key="val_post_xlsx"):
                st.download_button(
                    "Descargar Excel completo",
                    excel_all,
...
tabs = st.tabs(
    ["PRE BCP-txt", "rechazo IBK", "PRE BCP-xlsx", "POST BCP-xlsx"]
)
with tabs[0]:
    flujo_pre_bcp_txt()
with tabs[1]:
